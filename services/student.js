...
class StudentService extends BaseService {
    ...
    async createStudent(companyId, result) {
        ...
        const trimmedContact = contact.replace(/\D/g, '')
    
        const tempPassword = `${firstName}${trimmedContact}#`;
    ...
    async updateStudent(userId) {
        ...
        let trimmedContact = this.req.personal_info.contact.replace(/\D/g, '')
        trimmedContact  = trimmedContact.replace(/^\d+\s*/, '');

        const tempPassword = `${this.req.personal_info.firstName}${trimmedContact}#`;
        const encryptedPassword = await createHash(tempPassword);
